<template>
  <div
    class="fixed top-0 left-0 w-full flex justify-center inset-center z-20 bg-dark-light"
    :class="{'drop-shadow-xl bg-dark':isScrollMove}"
    id="navbar"
  >
    <div class="container h-20 flex items-center justify-between px-4">
      ...
      <nav class="flex gap-4">
        ...
        <ul class="flex gap-2">
          <li v-for="iconLink in iconLinkData" :key="iconLink.id">
            <a :href="iconLink.link" :title="`Przejdź do ${iconLink.title}`">
              <span
                class="block w-6 h-6 bg-auto bg-center"
                :style="`background-image:url(${iconLink.icon})`"
              ></span>
